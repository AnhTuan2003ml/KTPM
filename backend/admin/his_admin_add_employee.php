...
if (isset($_POST['add_doc'])) {
    ...
    $query_check_email = "SELECT COUNT(*) FROM his_docs WHERE doc_email = ?";
    $stmt_check_email = $mysqli->prepare($query_check_email);
    $stmt_check_email->bind_param('s', $doc_email);
    ...
    if ($count > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $err = "Failed to add employee details. Please try again later.";
        }
        $stmt->close();
    }
}
